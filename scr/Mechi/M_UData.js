...
const M_UData = ({navigation, route}) => {
  ...
  Login = id => {
    fetch(
      'https://timeparser.000webhostapp.com/Database/U_Get.php?U_id=' + id,
      {
        method: 'POST',
        headers: {
    ...
    <View style={{flex: 1}}>
      <View style={{flex: 1}}>
        ...
        <View style={{padding: 5}}>
          ...
          <Text style={styles.txt}>Name: - {M_Data?.U_Name}</Text>
          <Text style={styles.txt}>Email ID: - {M_Data?.U_Email}</Text>
          <Text style={styles.txt}>
            Phone Number:
            {M_Data?.U_Phone == '' ? '216-xxx-xxxx' : M_Data?.U_Phone}
          </Text>
          <Text style={styles.txt}>
            Work Done: - {M_Data?.U_Workdone == '' ? '0' : M_Data?.U_Workdone}
          </Text>
        </View>
      </View>
